...
/*	$(".btn-edit").click(function() {
	...
	$(".slide, .slides_container").css({
		width : win_w + "px"
	});
	$(".slides_container").css({
		"margin-left" : "-" + (win_w - 1200) / 2 + "px"
	});
	$('#slides').slides({
		preload : true,
	...
	$('.pagination li a').addClass("wkds-icon");
});*/
...
/*function Check() {
	if ($("#txtUserName").val() == "") {
		$('.txt-userMeg').show();
		$('.txt-userMeg').html("用户名不能为空！");
		$("#txtUserName").focus();
		return false;
	}
	if ($("#txtPassword").val() == "") {
		$("#txtPassword").show();
		$('.txt-pwdMsg').html("密码不能为空！");
		$("#txtPassword").focus();
		return false;
	}
	return true;
...
(function($) {
	...
	var ready = {
		getPath : function() {
			...
			var jsAllPath = jsPath.replace(
					jsPath.split('/')[jsPath.split('/').length - 1], '');
		...
		append : function() {
			var ie6PNG = '<!--[if IE 6]><script type="text/javascript" src="'
					+ this.getPath() + 'css/png.js"></script><![endif]-->';
			document.write('\n'
					+ '<link rel="stylesheet" type="text/css" href="'
					+ this.getPath() + 'css/vdform.css"  />' + '\n' + ie6PNG);
			this.global();
		},
		global : function() {
	...
	validator = {
		...
		isIdCard : function(number) {
			...
			case 15:
				if ((parseInt(number.substr(6, 2)) + 1900) % 4 == 0
						|| ((parseInt(number.substr(6, 2)) + 1900) % 100 == 0 && (parseInt(number
								.substr(6, 2)) + 1900) % 4 == 0)) {
					// 测试出生日期的合法性
			...
			case 18:
				 ...
				 */
				if (parseInt(number.substr(6, 4)) % 4 == 0
						|| (parseInt(number.substr(6, 4)) % 100 == 0 && parseInt(number
								.substr(6, 4)) % 4 == 0)) {
					// 闰年出生日期的合法性正则表达式
		...
		checkRepeat : function(value) {
			...
			if ((toval != "undefined" || toval != "")
				...
				validator.showTipMsg(element, str_errmsg, errindex,
						value == toval);
				return value == toval;
		...
		doajax : function(value) {
			...
			var Charset = jQuery.browser.msie ? document.charset
					: document.characterSet;
			var methodtype = (Charset.toLowerCase() == 'utf-8') ? 'post'
					: 'get';
			...
			var s = $.ajax({
				...
				success : function(data) {
					...
					if (data != 'success') {
						...
						return validator.showTipMsgByAjax(element, errmsg,
								false);
					}
					if (data == 'success') {
						return validator
								.showTipMsgByAjax(element, "校验成功", true);
					}
				}
			}).responseText;
			s = s.replace(/(^\s*)|(\s*$)/g, "");
	...
	validator.showTipMsgByAjax = function(element, msg, isSuccess) {
		...
		var mytip = jQuery('<div class="validator_msg_tip"><div class="'
				+ validator_msg_ico
				+ '"></div>'
				+ '<div class="validator_tip_msg_content">'
				+ msg
	...
	validator.showTipMsg = function(element, msg, errindex, isSuccess) {
		...
		var tipmsg = arr_tipmsg[errindex] ? arr_tipmsg[errindex]
				: arr_tipmsg[0];
		var offset = element.offset();
		var top = offset.top - xOffset;
		var left = offset.left + element.width() + 5;
...
$(function() {
	...
	function signControl() {
		var perForm = $('#per_form'),
		perHd_li = $('#per_tab_hd li'), 
		perForm_clild = perForm.find('form');

		$('.per-next').click(function() {
			var selectIdex = $('#selectIdentity').find('input:checked').parent().index();
			perForm.show().siblings().hide();
			perHd_li.removeClass('active').eq(1).addClass('active');
			perForm_clild.eq(selectIdex).show();
		});
		$('.per-prev').click(function() {
			//perForm.hide().prev().show();
			//perForm_clild.hide();
			//perHd_li.removeClass('active').eq(0).addClass('active');
			location.reload();
		});
		// $('.btn-datum').click(function(){
		// perForm.hide().next().show();
		// perHd_li.removeClass('active').eq(2).addClass('active');
		// });
	}
	;
	signControl();
